module.exports = {
    ...
    execute(msg,args,varstore,config,bot,db,customstatus){
        ...
        if(!args[1]){
            msg.channel.send({embeds:[varstore.embederror.setDescription(`invalid format! see ${config.PREFIX}help removestatus for more info`)]})
            return
        }
        if(isNaN(args[1]) == true){
            msg.channel.send({embeds:[varstore.embederror.setDescription('ID must be a number!')]})
            return
        }
        ...
        msg.channel.send({embeds:[varstore.embed.setDescription('Removed!\n'+`\`\`\`${JSON.stringify(customstatus[args[1]],null,'\t')}\`\`\``)]})
